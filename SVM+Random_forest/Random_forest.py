# -*- coding: utf-8 -*-
"""
Created on Fri Jun 18 00:00:13 2021
...
rf = RandomForestClassifier(n_estimators = 350,random_state=7, min_samples_split=50)
rf_fit=rf.fit(X_rd_train,y_rd_train)
rf_predict=rf_fit.predict(X_rd_test)
rf.score(X_rd_test, y_rd_test)

#evaluate

# accuracy score
acc = accuracy_score(y_rd_test, rf_predict)
print(f"Accuracy score: {round(acc,4)*100}%")

# precision score
precision = precision_score(y_rd_test, rf_predict,average='macro')
print(f"Precision score: {round(precision,4)*100}%")

# recall_score
recall = recall_score(y_rd_test, rf_predict,average='macro')
print(f"Recall score: {round(recall,4)*100}%")

# f1 score
f1 = f1_score(y_rd_test, rf_predict,average='macro')
print(f"f1 score: {round(f1,4)*100}%")

#confusion matrix
rf_matrix = confusion_matrix((y_rd_test), rf_predict)
sns.heatmap(pd.DataFrame(rf_matrix), annot=True,cmap="Reds" , fmt='g')
plt.tight_layout()
